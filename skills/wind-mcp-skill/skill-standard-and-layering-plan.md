...
### 4.10 评审过程中被推翻的两个论断（记录在案）

| 原论断 | 为什么错 | 修正 |
| --- | --- | --- |
| 「替换 `question` 参数说明会丢失路由信息，所以不能换」 | 把结果当成了原则。路由本来就**不该**写在参数格里，工具描述才是它的位置 | 真实风险是**净删除**（现在路由只存在于参数格），不是位置原则。配套搬迁后即可替换 → A9 |
| 「显式传选填参数的真默认值会增加出错面」 | 过头了。`lang` / `period` / `aftype` / `issusp` 显式传基本无害（cli 会归一化）。只有 `indexes` 成立：8 个中文字段名逐字抄写易错、违反门禁 5、多取数据 | 改标「示例」的真实损失是**删掉了"省略会发生什么"这条信息**（模型不知道 `period` 省略等于 `1d`，可能改为猜值或反问用户），不是"显式传危险" → A8 按 `required` 分列渲染 |
...
契约表由 `cli.mjs:1422` **本地渲染**（`${tool.description}` + 从 inputSchema 逐行生成），
且 `cli.mjs:1404-1407` 已有针对 `begin_date` / `end_date` 的**本地 description 覆写先例**。因此：

- **不需要等后端改 `tools/list`**，在渲染层覆写即可；
- 改完跑一次 `sync-contracts` 就能看到效果，不满意再调 —— **可迭代、可回退，不是一次性决策**；
- ⚠️ **覆写必须写在 `cli.mjs` 里**。手改 `references/*.md` 的 BEGIN/END 区块会被下次同步覆盖；
- ⚠️ **A9 必须同时做两半**。只替换参数说明而不把路由搬到工具描述 = 净删除（stock.md 里 10 个工具有 7 个用 `question`，全靠这段文字区分）；
- A8 是结构化判断（读 `required`），不是中文正则，**不适用 §4.7 对文本转换的脆弱性论证**。

**A9 待解决的已知矛盾**：`get_risk_metrics` 声明窗口固定 100周/24月/60月，但示例与正面场景都写「过去1年」——没有这一档。
照抄示例可能拿回 100 周口径数据而不自知（正是门禁 9 想防的口径错误）。**这条会产出错误数字，优先级高于措辞优化。**
